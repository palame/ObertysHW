...
def get_paths_and_texts(is_words):
    paths_and_texts = []
    if is_words:
        with open('C:\\Users\\PIN\\Desktop\\Handwriting-OCR-master/IAM_words/words.txt') as f:
            for line in f:
                ...
                img_path = os.path.join('C:\\Users\\PIN\\Desktop\\Handwriting-OCR-master/IAM_words/words', label_dir, sub_label_dir, fn)
                gt_text = ' '.join(line_split[8:])
                paths_and_texts.append([img_path, gt_text])
    else:
        with open('C:\\Users\\PIN\\Desktop\\Handwriting-OCR-master/IAM_lines/lines.txt') as f:
            for line in f:
                if not line or line.startswith('#'):
                    continue
                line_split = line.strip().split(' ')
                ...
                sub_label_dir = '{}-{}'.format(file_name_split[0], file_name_split[1])
                fn = '{}.png'.format(line_split[0])
                img_path = os.path.join('C:\\Users\\PIN\\Desktop\\Handwriting-OCR-master/IAM_lines/lines', label_dir, sub_label_dir, fn)
                gt_text = ' '.join(line_split[8:])
